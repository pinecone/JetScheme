...
(define (offset-momentum!)
  (let loop ((i 0) (px 0.0) (py 0.0) (pz 0.0))
    (if (= i NBODIES)
        (let ((s (ref BODIES 0)))
          (setf! s 'vx (- (/ px SOLAR-MASS)))
          (setf! s 'vy (- (/ py SOLAR-MASS)))
          (setf! s 'vz (- (/ pz SOLAR-MASS))))
        (let* ((b (ref BODIES i))
               (m (ref b 'mass)))
          (loop (+ i 1)
                (+ px (* (ref b 'vx) m))
          ...
          (let j-loop ((j (+ i 1)))
            (if (< j NBODIES)
                (let* ((bj (ref BODIES j))
                       ...
                       (mi (ref bi 'mass))
                       (mj (ref bj 'mass)))
                  (setf! bi 'vx (- (ref bi 'vx) (* dx mj mag)))
                  (setf! bi 'vy (- (ref bi 'vy) (* dy mj mag)))
                  (setf! bi 'vz (- (ref bi 'vz) (* dz mj mag)))
                  (setf! bj 'vx (+ (ref bj 'vx) (* dx mi mag)))
                  (setf! bj 'vy (+ (ref bj 'vy) (* dy mi mag)))
                  (setf! bj 'vz (+ (ref bj 'vz) (* dz mi mag)))
                  (j-loop (+ j 1)))))
          (i-loop (+ i 1)))))
  (let pos-loop ((i 0))
    (if (< i NBODIES)
        (let ((b (ref BODIES i)))
          (setf! b 'x (+ (ref b 'x) (* dt (ref b 'vx))))
          (setf! b 'y (+ (ref b 'y) (* dt (ref b 'vy))))
          (setf! b 'z (+ (ref b 'z) (* dt (ref b 'vz))))
          (pos-loop (+ i 1))))))
