...
(define (tree-left-rotate! t x)
  (let ((y (ref x 'right))
        (xp (ref x 'parent)))
    (setf! x 'right (ref y 'left))
    (when (ref y 'left) (setf! (ref y 'left) 'parent x))
    (setf! y 'parent xp)
    (cond
      ((not xp) (setf! t 'root y))
      ((eq? x (ref xp 'left))  (setf! xp 'left  y))
      (else                    (setf! xp 'right y)))
    (setf! y 'left x)
    (setf! x 'parent y)
    y))
...
(define (tree-right-rotate! t y)
  (let ((x (ref y 'left))
        (yp (ref y 'parent)))
    (setf! y 'left (ref x 'right))
    (when (ref x 'right) (setf! (ref x 'right) 'parent y))
    (setf! x 'parent yp)
    (cond
      ((not yp) (setf! t 'root x))
      ((eq? y (ref yp 'left))  (setf! yp 'left  x))
      (else                    (setf! yp 'right x)))
    (setf! x 'right y)
    (setf! y 'parent x)
    x))
      ...
      (cond
        ((not x)
         (let ((z (make-rbnode k v)))
           ...
           (cond
             ((not y) (setf! t 'root z))
             ((< (cmp k (ref y 'key)) 0) (setf! y 'left z))
             (else (setf! y 'right z)))
           (cons #t z)))
        (else
       ...
       (let* ((y (if (or (not (ref z 'left)) (not (ref z 'right)))
         ...
         (cond
           ((not (eq? y z))
            ...
            (setf! y 'parent (ref z 'parent))
            (setf! y 'color  (ref z 'color))
            (setf! y 'left   (ref z 'left))
            (setf! y 'right  (ref z 'right))
            (when (ref z 'left)
              (setf! (ref z 'left)  'parent y))
            (when (ref z 'right)
              (setf! (ref z 'right) 'parent y))
            (cond
              ((ref z 'parent)
               (cond
              ...
              (else
               (let ((w2 (cond
                           ((or (not (ref w 'right))
                            ...
                            (ref xparent 'right))
                           (else w))))
                 (setf! w2 'color (ref xparent 'color))
                 (setf! xparent 'color 'black)
                 (when (ref w2 'right)
                   (setf! (ref w2 'right) 'color 'black))
                 (tree-left-rotate! t xparent)
                 (let ((nx (ref t 'root)))
            ...
            (cond
              ...
              (else
               (let ((w2 (cond
                 ...
                 (setf! xparent 'color 'black)
                 (when (ref w2 'left)
                   (setf! (ref w2 'left) 'color 'black))
                 (tree-right-rotate! t xparent)
                 (let ((nx (ref t 'root)))
                   (loop nx (and nx (ref nx 'parent)))))))))))
      (else
       (when x (setf! x 'color 'black))))))
        ...
        (else
         (let ((bx (tree-get voxelmap nv)))
           (cond
             ...
             (else
              (setf! bx 'v (cons motion (ref bx 'v))))))
