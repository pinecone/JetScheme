...
(define (var-add-c! v c)
  (setf! (ref v 'constraints) (cons c (ref v 'constraints))))

(define (var-remove-c! v c)
  (setf! (ref v 'constraints) (list-without (ref v 'constraints) c))
  (if (eq? (ref v 'determined-by) c)
      (setf! (ref v 'determined-by) '())))
...
(define (c-execute! c)
  (let ((k (ref c 'kind)))
    (cond
      ...
      ((= k K-EQUALITY)
       (if (= (ref c 'dir) DIR-FORWARD)
           (setf! (ref (ref c 'v2) 'value) (ref (ref c 'v1) 'value))
           (setf! (ref (ref c 'v1) 'value) (ref (ref c 'v2) 'value))))
      ((= k K-SCALE)
       (let ((scale (ref (ref c 'scale) 'value))
             (offset (ref (ref c 'offset) 'value)))
         (if (= (ref c 'dir) DIR-FORWARD)
             (setf! (ref (ref c 'v2) 'value)
                   (+ (* (ref (ref c 'v1) 'value) scale) offset))
             (setf! (ref (ref c 'v1) 'value)
...
(define (new-scale! src scale offset dst strength planner)
  (let ((c (make-constraint K-SCALE strength)))
    ...
    (setf! (ref c 'v2) dst)
    (setf! (ref c 'dir) DIR-NONE)
    (setf! (ref c 'scale) scale)
    (setf! (ref c 'offset) offset)
    (planner-add-constraint! planner c)
    c))
...
(define (planner-remove-propagate-from! planner out)
  (setf! (ref out 'determined-by) '())
  (setf! (ref out 'walk-strength) ABSOLUTE-WEAKEST)
  (setf! (ref out 'stay) #t)
...
(define (chain-test n)
  (let ((planner (make-planner))
    ...
    (let loop ((i 0))
      (if (<= i n)
          (begin (setf! (ref vars i) (make-var 0)) (loop (+ i 1)))))
    (let loop ((i 0))
      (if (< i n)
          (begin
