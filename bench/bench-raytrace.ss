        ...
        (let ((t (- (/ (+ (vector-dot (ref self 'position) (ref ray 'position)) (ref self 'd)) Vd))))
          (if (<= t 0)
              ...
              (begin
                ...
                (setf! info 'color (material-get-color (ref self 'material) 0.0 0.0))
                (if (ref self 'material 'has-texture)
                    (let* ((vU (make-vector (ref self 'position 'y)
                                            (ref self 'position 'z)
                                            (- (ref self 'position 'x))))
...
(define (engine-ray-trace self info ray scene depth)
  (let ((col (color-multiply-scalar (ref info 'color) (ref scene 'background 'ambience)))
        (shininess (expt 10 (+ (ref info 'shape 'material 'gloss) 1))))
    (let light-loop ((i 0))
      (if (>= i (vector-length (ref scene 'lights)))
          ...
          (let* ((light (vector-ref (ref scene 'lights) i))
            ...
            (if (and (<= depth (ref self 'ray-depth))
                     ...
                     (> (ref info 'shape 'material 'reflection) 0))
                (let* ((reflection-ray (engine-get-reflection-ray self (ref info 'position) (ref info 'normal) (ref ray 'direction)))
                  ...
                  (if (and (ref refl 'is-hit) (> (ref refl 'distance) 0))
                      ...
                      (setf! refl 'color (ref scene 'background 'color)))
                  (set! col (color-blend col (ref refl 'color) (ref info 'shape 'material 'reflection))))
                #f)
                  ...
                  (let* ((shadow-ray (make-ray (ref info 'position) v))
                    ...
                    (if (and (ref shadow-info 'is-hit) (not (eq? (ref shadow-info 'shape) (ref info 'shape))))
                        (let* ((vA (color-multiply-scalar col 0.5))
                               (dB (* 0.5 (expt (ref shadow-info 'shape 'material 'transparency) 0.5))))
                          (set! col (color-add-scalar vA dB)))
                        #f))
                  #f)
              (if (and (ref self 'render-highlights)
                       (not (ref shadow-info 'is-hit))
                       (> (ref info 'shape 'material 'gloss) 0))
                  (let* ((Lv (vector-normalize (vector-subtract (ref info 'shape 'position) (ref light 'position))))
                         (E (vector-normalize (vector-subtract (ref scene 'camera 'position) (ref info 'shape 'position))))
