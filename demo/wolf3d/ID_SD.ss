...
(define smm_Off 0)
(define smm_AdLib 1)
(define AdLibRegisters (make-bytevector 256 0))
(define MusicMode smm_Off)
(define sqActive #f)
(define alSound #f)
(define alFXReg 0)

(define (SDL_SetInstrument track which instrument percussive) #f)

(define (SDL_ALStopSound)
  (set! alSound #f)
  (alOut #xb0 0))

(define (SDL_AlSetFXInst instrument) #f)

(define (SDL_ALSoundService) #f)

(define (SDL_ALService) #f)
...
(define (SDL_DetectAdLib) #f)

(define (SDL_t0Service) #f)

(define (SDL_ALPlaySound sound)
  (SDL_ALStopSound)
  (let* ((data (sound-chunk sound))
         (modifier-sustain (ref data (+ SOUNDCOMMONSIZE 6)))
         (carrier-sustain (ref data (+ SOUNDCOMMONSIZE 7))))
    (when (= (bitwise-ior modifier-sustain carrier-sustain) 0)
      (Quit "SDL_ALPlaySound() - Bad instrument")))
  #f)

(define (alOut register value)
  (setf! AdLibRegisters register value))

(define SD_MusicOff
  (lambda preserve-current
    (when (= MusicMode smm_AdLib)
      ...
      (let mute ((track 0))
        (when (< track 10)
          ...
          (mute (+ track 1)))))
    (set! sqActive #f)
    (unless (and (pair? preserve-current) (car preserve-current))
      (set! current-music #f))))
(define (SD_MusicOn)
  (set! sqActive #t))
...
(define (SD_SetSoundMode mode)
  ...
  (when (= mode sdm_AdLib)
    (set! mode sdm_PC))
  (let ((result (or (= mode sdm_Off) (= mode sdm_PC))))
    (when (and result (not (= mode SoundMode)))
      (SDL_ShutDevice)
      (set! SoundMode mode)
      (set! SoundTable STARTPCSOUNDS)
      (SDL_StartDevice))
    (SDL_SetTimerSpeed)
    result))
...
(define (SD_Default gotit sound-mode music-mode)
  (let ((got-sound gotit)
        (got-music gotit))
    (when (and got-sound (= sound-mode sdm_AdLib))
      (set! got-sound #f))
    (unless got-sound
      (set! sound-mode sdm_PC))
    (unless (= sound-mode SoundMode)
      (SD_SetSoundMode sound-mode))
    (when (and got-music (= music-mode smm_AdLib))
      (set! got-music #f))
