...
(define (shape-column page leftpix sourcex screenx rows top shade emissive)
              ...
              (let ((start (max 0 (+ top (truncate (/ (* texel rows) 64)))))
                ...
                (when (< start limit)
                  (let ((color (plus-sprite-color (ref page (+ topofs texel)) shade emissive)))
                    (let fill ((y start))
                      (when (< y limit)
                        (setf! framebuffer (+ (* (+ y viewtop) screenwidth) viewleft screenx) color)
                        (set! sprite-pixels (+ sprite-pixels 1))
                        (fill (+ y 1)))))))
              (texels (+ texel 1)))))
        (posts (+ offset 6))))))

(define (draw-run page leftpix sourcex x width rows top shade emissive)
  (let loop ((column 0))
    (when (< column width)
      (shape-column page leftpix sourcex (+ x column) rows top shade emissive)
      (loop (+ column 1)))))
...
(define (place-run page leftpix sourcex x width rows top height clip outward-left shade emissive)
  (let ((run (if clip (trim-run x width height outward-left) (list x width #f))))
    (when (> (cadr run) 0)
      (draw-run page leftpix sourcex (car run) (cadr run) rows top shade emissive))
    (caddr run)))

(define (scale-shape screenx shapenum height clip tilex tiley)
  (let* ((page (or (and clip plus-enabled (ref plus-sprites shapenum))
    ...
    (when (and (> scale 0) (<= scale maxscale))
      (let* ((rows (* scale 2))
        ...
        (if (and clip plus-enabled (< rows 64))
                  ...
                  (let ((width (column-width rows sourcex)))
                    (if (= width 0)
                        ...
                        (let* ((clamped (if (> slinex viewwidth) width (if (> width slinex) slinex width)))
                          ...
                          (if (< visible 1)
                              (left (- sourcex 1) x)
                              (unless (place-run page leftpix sourcex x visible rows top height clip #t shade
                                                 emissive)
                                (left (- sourcex 1) x))))))))
                        ...
                        (else
                         (let* ((start (if (< x 0) 0 x))
                                (visible (cond ((< x 0) (+ width x))
                                               ...
                                               (else width))))
                           (unless (place-run page leftpix next start visible rows top height clip #f shade
                                              emissive)
