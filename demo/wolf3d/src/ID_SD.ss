...
(define (InitDigiMap)
  (let loop ((index 0))
    (when (< index (vector-length wolfdigimap))
      (let ((sound (ref wolfdigimap index))
            (sample (ref wolfdigimap (+ index 1))))
        (setf! DigiMap sound -1)
        (when (< sample NumDigi)
          (let ((start (+ PMSoundStart (ref DigiList (* sample 2))))
                (length (ref DigiList (+ (* sample 2) 1))))
            (when (and (> length 0)
                       (let pages ((page start) (remaining length))
                         (or (<= remaining 0)
                             (and (PM_HasPage page)
                                  (pages (+ page 1) (- remaining PMPageSize))))))
              (setf! DigiMap sound sample)))))
      (loop (+ index 2)))))
