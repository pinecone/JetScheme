...
(define (DrawPlayBorderSides)
  (let ((left (- viewleft screenoffset))
        (bottom (+ viewtop viewheight)))
    (VWB_Bar (- screenoffset) 0 (- viewleft 1) (- screenheight STATUSLINES) 127)
...
(define (DrawPlayBorder)
  (VWB_Bar (- screenoffset) 0 screenwidth (- screenheight STATUSLINES) 127)
  (let* ((left (- viewleft screenoffset))
...
(define (DrawPlayScreen)
  ...
  (DrawPlayBorder)
  (when (> screenoffset 0)
    (VWB_Bar (- screenoffset) (- screenheight STATUSLINES) screenwidth STATUSLINES 127))
