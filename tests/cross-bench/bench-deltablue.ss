...
(define (var-add-c! v c)
  (set! (ref v 'constraints) (cons c (ref v 'constraints))))

(define (var-remove-c! v c)
  (set! (ref v 'constraints) (list-without (ref v 'constraints) c))
  (if (eq? (ref v 'determined-by) c)
      (set! (ref v 'determined-by) '())))
...
(define (c-execute! c)
  (let ((k (ref c 'kind)))
    (cond
      ...
      ((= k K-EQUALITY)
       (if (= (ref c 'dir) DIR-FORWARD)
           (set! (ref (ref c 'v2) 'value) (ref (ref c 'v1) 'value))
           (set! (ref (ref c 'v1) 'value) (ref (ref c 'v2) 'value))))
      ((= k K-SCALE)
       (let ((scale (ref (ref c 'scale) 'value))
             (offset (ref (ref c 'offset) 'value)))
         (if (= (ref c 'dir) DIR-FORWARD)
             (set! (ref (ref c 'v2) 'value)
                   (+ (* (ref (ref c 'v1) 'value) scale) offset))
             (set! (ref (ref c 'v1) 'value)
...
(define (new-scale! src scale offset dst strength planner)
  (let ((c (make-constraint K-SCALE strength)))
    ...
    (set! (ref c 'v2) dst)
    (set! (ref c 'dir) DIR-NONE)
    (set! (ref c 'scale) scale)
    (set! (ref c 'offset) offset)
    (planner-add-constraint! planner c)
    c))
...
(define (planner-remove-propagate-from! planner out)
  (set! (ref out 'determined-by) '())
  (set! (ref out 'walk-strength) ABSOLUTE-WEAKEST)
  (set! (ref out 'stay) #t)
...
(define (chain-test n)
  (let ((planner (make-planner))
    ...
    (let loop ((i 0))
      (if (<= i n)
          (begin (set! (ref vars i) (make-var 0)) (loop (+ i 1)))))
    (let loop ((i 0))
      (if (< i n)
          (begin
