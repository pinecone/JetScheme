...
(define (offset-momentum!)
  (let loop ((i 0) (px 0.0) (py 0.0) (pz 0.0))
    (if (= i NBODIES)
        (let ((s (ref BODIES 0)))
          (set! (ref s 'vx) (- (/ px SOLAR-MASS)))
          (set! (ref s 'vy) (- (/ py SOLAR-MASS)))
          (set! (ref s 'vz) (- (/ pz SOLAR-MASS))))
        (let* ((b (ref BODIES i))
               (m (ref b 'mass)))
          (loop (+ i 1)
                (+ px (* (ref b 'vx) m))
          ...
          (let j-loop ((j (+ i 1)))
            (if (< j NBODIES)
                (let* ((bj (ref BODIES j))
                       ...
                       (mi (ref bi 'mass))
                       (mj (ref bj 'mass)))
                  (set! (ref bi 'vx) (- (ref bi 'vx) (* dx mj mag)))
                  (set! (ref bi 'vy) (- (ref bi 'vy) (* dy mj mag)))
                  (set! (ref bi 'vz) (- (ref bi 'vz) (* dz mj mag)))
                  (set! (ref bj 'vx) (+ (ref bj 'vx) (* dx mi mag)))
                  (set! (ref bj 'vy) (+ (ref bj 'vy) (* dy mi mag)))
                  (set! (ref bj 'vz) (+ (ref bj 'vz) (* dz mi mag)))
                  (j-loop (+ j 1)))))
          (i-loop (+ i 1)))))
  (let pos-loop ((i 0))
    (if (< i NBODIES)
        (let ((b (ref BODIES i)))
          (set! (ref b 'x) (+ (ref b 'x) (* dt (ref b 'vx))))
          (set! (ref b 'y) (+ (ref b 'y) (* dt (ref b 'vy))))
          (set! (ref b 'z) (+ (ref b 'z) (* dt (ref b 'vz))))
          (pos-loop (+ i 1))))))
