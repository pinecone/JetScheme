...
# Recursive descent over a bytewise corpus; arrays => lists, objects =>
# dicts, null => JSON_NULL sentinel. Hand-rolled to mirror the .lua/.ss
# ports (i.e. measuring interpreter speed, not the C json module).
...
def parse_string_esc(start, at):
    pieces = [corpus[start:at].decode("ascii")]
    k = at
    while True:
        c = corpus[k]
        if c == 34:
        ...
        if c == 92:
            pieces.append(_ESCAPES[corpus[k + 1]])
            k += 2
        else:
            pieces.append(chr(c))
            k += 1
...
def parse_string(start):
    ...
    while True:
        c = corpus[j]
        if c == 34:
            return corpus[start:j].decode("ascii"), j + 1
        if c == 92:
            return parse_string_esc(start, j)
        j += 1
...
def parse_number(i):
    ...
    if corpus[i] == 45:  # -
        i += 1
    i = skip_digits(i)
    if i < n and corpus[i] == 46:  # .
        i = skip_digits(i + 1)
    i = skip_exp(i)
    s = corpus[start:i].decode("ascii")
...
def parse_object(i):
    ...
    while True:
        i = skip_ws(i)
        key, ki = parse_string(i + 1)  # skip leading "
        ki = skip_ws(ki)
        ki = ki + 1  # skip :
        v, ni = parse_value(ki)
        obj[key] = v
...
head = root["head"]
ops = root["operations"]
if isinstance(head, dict) and isinstance(ops, list) and len(ops) == 156:
    print("ok")
else:
