...
skip_digits = function(i)
  while i <= n do
    local c = corpus:byte(i)
    if c >= 48 and c <= 57 then i = i + 1 else return i end
  end
  return i
end
...
skip_exp = function(i)
  ...
  local c = corpus:byte(i)
  if c ~= 101 and c ~= 69 then return i end  -- e E
  i = i + 1
  c = corpus:byte(i)
  if c == 43 or c == 45 then i = i + 1 end   -- + -
  return skip_digits(i)
end

local function decode_escape(c)
  if c == 34 then return "\""
  elseif c == 92 then return "\\"
  elseif c == 47 then return "/"
  elseif c == 98 then return "\b"
...
parse_string_esc = function(start, at)
  local pieces = { corpus:sub(start, at - 1) }
  local k = at
  while true do
    local c = corpus:byte(k)
    if c == 34 then return table.concat(pieces), k + 1 end
    if c == 92 then
      pieces[#pieces + 1] = decode_escape(corpus:byte(k + 1))
      k = k + 2
    else
      pieces[#pieces + 1] = corpus:sub(k, k)
      k = k + 1
    end
  end
end

parse_string = function(start)
  local j = start
  while true do
    local c = corpus:byte(j)
    if c == 34 then return corpus:sub(start, j - 1), j + 1 end
    if c == 92 then return parse_string_esc(start, j) end
    j = j + 1
  end
end

parse_number = function(i)
  local start = i
  if corpus:byte(i) == 45 then i = i + 1 end  -- -
  i = skip_digits(i)
  if i <= n and corpus:byte(i) == 46 then     -- .
    i = skip_digits(i + 1)
  end
  i = skip_exp(i)
  return tonumber(corpus:sub(start, i - 1)), i
end
...
parse_array = function(i)
  i = skip_ws(i)
  if corpus:byte(i) == 93 then return {}, i + 1 end  -- ]
  local arr = {}
  while true do
    local v, ni = parse_value(i)
    arr[#arr + 1] = v
    ni = skip_ws(ni)
    local c = corpus:byte(ni)
    if c == 44 then i = ni + 1                       -- ,
    elseif c == 93 then return arr, ni + 1           -- ]
    else error("bad array") end
  end
end

parse_object = function(i)
  i = skip_ws(i)
  if corpus:byte(i) == 125 then return {}, i + 1 end  -- }
  local obj = {}
  while true do
    i = skip_ws(i)
    ...
    local c = corpus:byte(ni)
    if c == 44 then i = ni + 1                        -- ,
    elseif c == 125 then return obj, ni + 1           -- }
...
for _ = 1, ITERS do parse() end

local root = parse()
local head = root.head
local ops = root.operations
if type(head) == "table" and type(ops) == "table" and #ops == 156 then
  print("ok")
else
  print("verify failed")
  os.exit(1)
